...
class BaseService(ABC):
    ...
    def _get_or_404(self, model_class, entity_id: int, error_message: str = None):
    ...
    def _get_by_public_id_or_404(self, model_class, public_id: str):
        """Get entity by public_id or raise 404 error"""
        entity = self.db.query(model_class).filter(model_class.public_id == public_id).first()
        if not entity:
            resource_type = model_class.__name__
            raise NotFoundError(resource_type, public_id)
        return entity
    
    def _validate_unique_field(self, model_class, field_name: str, field_value: Any, 
                              exclude_id: int = None, error_message: str = None):
        """Validate that field value is unique"""
        query = self.db.query(model_class).filter(getattr(model_class, field_name) == field_value)
        if exclude_id:
    ...
    def _apply_search(self, query, model_class, search_term: str, search_fields: List[str]):
    ...
    def _apply_sorting(self, query, model_class, sort_by: str = None, 
    ...
    def _send_notification(self, notification_type: str, recipients: List[str], 
...
class CRUDService(BaseService):
    ...
    def get_by_id(self, entity_id: int) -> Dict[str, Any]:
        """Get entity by ID"""
        entity = self._get_or_404(self.model_class, entity_id)
        return self.response_schema().dump(entity)
    
    def get_by_public_id(self, public_id: str) -> Dict[str, Any]:
        """Get entity by public_id"""
        entity = self._get_by_public_id_or_404(self.model_class, public_id)
        return self.response_schema().dump(entity)
    
    def list(self, page: int = 1, per_page: int = 20, search: str = None,
        ...
        if search and hasattr(self, 'search_fields'):
            query = self._apply_search(query, self.model_class, search, self.search_fields)
        
        # Apply sorting
        query = self._apply_sorting(query, self.model_class, sort_by, sort_order)
    ...
    def activate(self, entity_id: int) -> Dict[str, Any]:
